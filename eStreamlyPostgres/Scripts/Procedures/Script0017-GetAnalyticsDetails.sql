CREATE PROCEDURE [dbo].[GetAnalyticsDetails]
AS 
BEGIN
select up.UniqueId
	  ,ul.Name EventName
	  ...
	  ,MAX(ISNULL(co.orderCount,0)) OrderCount
	  ,CAST(MAX(ISNULL(TotalRevenue,0)) / 100  as decimal(10,2)) Revenue
	  ,MAX(u.FullName) EventHost
	  ,MAX(ISNULL(b.BuyClicks,0)) BuyClicks
